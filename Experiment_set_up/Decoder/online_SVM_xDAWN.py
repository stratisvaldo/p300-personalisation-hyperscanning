''''
Example:
python Experiment_set_up/Decoder/online_svm_xdawn_group.py `
  ...
  --model_path models_svm/svm_xdawn_group_calibration.joblib `
  --tmin 0.0 `
  --tmax 0.8 `
  --baseline 0.0 `
  --lowcut 0.1 `
  ...
  --downsample_factor 4 `
  --debug `
  --save_decisions data_test/testing_play_group_svm_xdawn_decisions.npz `
  --save_eeg_data data_test/testing_play_group_svm_xdawn_eeg.npz
'''

'''
- Connect to multiple Unicorn EEG streams
- Connect to marker stream from free-speller
- Load trained grouped xDAWN + SVM model and saved participant-wise scaling stats
- For every flash_on:
    - collect one epoch from each participant stream
    - preprocess each participant exactly like training:
        bandpass filter -> baseline correct -> standardise -> downsample -> xDAWN
    - flatten each participant's xDAWN-projected epoch
...
Online preprocessing matches group_epochML_xdawn.py + train_svm_xdawn.py exactly:
    bandpass (continuous) -> baseline correct -> standardise (saved scaler) ->
    downsample -> xDAWN (saved filters) -> flatten -> concatenate -> SVM
...
def make_decision_outlet():
    info = StreamInfo(
        ...
        source_id="p300-online-group-svm-xdawn-decoder"
    )
...
def apply_xdawn_filters_online(epoch, filters):
    """
    Apply xDAWN spatial filters to a single epoch at inference time.

    Mirrors the training transform in train_svm_xdawn.py:
        training:  np.einsum('nct,ck->nkt', epochs_3d, filters)
        online:    epoch @ filters
    Both project the channel axis onto the learned spatial components.

    Parameters
    ----------
    epoch   : ndarray, shape (T, C)
              One participant's preprocessed, standardised, downsampled epoch.
    filters : ndarray, shape (C, n_components)
              Spatial filter matrix loaded from the model package.

    Returns
    -------
    ndarray, shape (T, n_components)
    """
    return (epoch @ filters).astype(np.float32)
...
def main():
    ...
    parser.add_argument("--model_path", type=str, required=True,
                        help="Path to xDAWN + SVM model .joblib produced by train_svm_xdawn.py")
    ...
    print("\nLoading grouped xDAWN + SVM model...")
    model_package = joblib.load(args.model_path)

    if not isinstance(model_package, dict):
        raise RuntimeError("Expected .joblib to contain a dict. Use train_svm_xdawn.py to produce the model.")

    for required_key in ("svm", "n_features", "n_xdawn_components", "xdawn_filters"):
        if required_key not in model_package:
            raise RuntimeError(
                f"Model package is missing '{required_key}'. "
                f"Ensure the model was produced by train_svm_xdawn.py."
            )
    ...
    participant_scaler_means  = model_package["participant_scaler_means"]
    participant_scaler_scales = model_package["participant_scaler_scales"]
    n_xdawn_components     = int(model_package["n_xdawn_components"])
    xdawn_filters_list     = model_package["xdawn_filters"]

    if n_participants_model is not None and n_participants_model != n_participants_runtime:
        raise RuntimeError(
            f"Model was trained with {n_participants_model} participants but "
            f"runtime provides {n_participants_runtime} streams."
        )

    if len(participant_scaler_means) != n_participants_runtime:
        raise RuntimeError("Number of saved participant scalers does not match runtime number of participants.")

    if len(xdawn_filters_list) != n_participants_runtime:
        raise RuntimeError(
            f"Number of saved xDAWN filter matrices ({len(xdawn_filters_list)}) "
            f"does not match runtime number of participants ({n_participants_runtime})."
        )

    # Compute expected feature count from runtime parameters and model xDAWN components.
    # Per participant: n_xdawn_components * n_times_after_downsample
    # (xDAWN replaces the channel dimension with n_components)
    ...
    features_per_participant = n_xdawn_components * n_times_after_downsample
    ...
    print("xDAWN components per participant:", n_xdawn_components)
    print("Runtime participants           :", n_participants_runtime)
    print("Runtime n_times               :", n_times_runtime)
    print("Runtime n_times after downsample:", n_times_after_downsample)
    ...
    if n_features_runtime != n_features_model:
        raise RuntimeError(
            ...
            f"Check tmin/tmax/expected_srate/downsample_factor match training."
        )
    ...
    print("\nGrouped xDAWN decoder is running...\n")
            ...
            while True:
                ...
                if parsed["event"] == "flash_on":
                    ...
                    for p in range(n_participants_runtime):
                        ...
                        # standardise -> downsample -> xDAWN
                        # order matches group_epochML_xdawn.py + train_svm_xdawn.py exactly
                        ...
                        epoch = downsample_epoch(epoch, args.downsample_factor)  # (T_down, C)

                        epoch = apply_xdawn_filters_online(epoch, xdawn_filters_list[p])  # (T_down, n_comp)
                    ...
                    # flatten each participant's (T_down, n_comp) and concatenate
                    grouped_parts = [
                    ...
                    print(
                        f"Grouped xDAWN flash scored | kind={parsed['kind']} idx={parsed['idx']} "
    ...
    if args.save_decisions is not None:
        ...
        print("\nSaved grouped xDAWN decoder decisions to:", args.save_decisions)
    ...
    if args.save_eeg_data is not None:
        ...
        np.savez(
            ...
            # raw/filtered epochs saved pre-xDAWN as (N_flashes, P, T, C)
            # so they can be re-used for offline analysis with any number of components
