''''
Example:
python Experiment_set_up/Epoch_extraction/group_epochML_xdawn.py `
  --input data_calib/group_calibration_recording.npz `
  --output epoched_data/p300_group_epochs_xdawn.npz `
  --tmin 0.0 `
  --tmax 0.8 `
  --baseline 0.0 `
  --lowcut 0.1 `
...
# Difference from group_epochML.py:
#   X is saved as (N, n_participants, n_channels, n_times_downsampled)
#   instead of being flattened to (N, P*C*T).
#   This allows train_svm_xdawn.py to fit xDAWN spatial filters directly on
#   each participant's channel structure without any reshaping.
...
8. downsample
9. stack grouped epoch to (N, P, C, T) — NOT flattened
10. save X, y and metadata

One trial = (P, C, T) — all participants stacked along a new axis.
...
def main():
    ...
    parser.add_argument("--output", type=str, default="p300_group_epochs_xdawn.npz")
    ...
    n_times_downsampled = len(np.arange(0, expected_len, args.downsample_factor))
    ...
    print(f"Times after downsample : {n_times_downsampled}")
    print(f"Output X shape         : (N, {n_participants}, {n_channels}, {n_times_downsampled})")
    print("Participant order      : fixed input order")

    # Each element will be (P, C, T) for one trial; stacked to (N, P, C, T) at the end
    X = []
    y = []
    ...
    for i in range(len(event_names)):
        ...
        # epoch_per_participant collects (C, T) arrays, one per participant.
        # Stored as (C, T) = transposed from the (T, C) processing format
        # so that stacking gives (P, C, T) directly.
        ...
        for p in range(n_participants):
            ...
            # Transpose to (C, T_down) so stacking across participants gives (P, C, T)
            epoch_per_participant.append(epoch.T.astype(np.float32))
        ...
        # Stack participants: list of (C, T) -> (P, C, T)
        grouped_epoch = np.stack(epoch_per_participant, axis=0)  # (P, C, T)
    ...
    save_dict = {
        # X is (N, n_participants, n_channels, n_times_downsampled)
        # train_svm_xdawn.py can index directly: X[:, p, :, :] -> (N, C, T) for participant p
        "X": X,
        "y": y,
        ...
        "n_times_downsampled": np.array([n_times_downsampled], dtype=np.int32),
    ...
    print("\nSaved grouped xDAWN epoch dataset to:", args.output)
    print("X shape      :", X.shape, " (N, P, C, T)")
    ...
    print(f"n_times_downsampled  : {n_times_downsampled}")
